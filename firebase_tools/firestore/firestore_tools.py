...
def get_document(collection_name: str, document_name: str) -> Union[None, dict]:
    ...
    doc = doc_ref.get()
    if doc.exists:
        logger.info(f"[Firestore READ] {doc_ref.path} (exists=true) (read=1)")
        return doc.to_dict()
    logger.info(f"[Firestore READ] {doc_ref.path} (exists=false) (read=1)")
    return None
...
def get_documents(
    ...
    logger.info(
        f"[Firestore READ] {collection_name} (condition={condition}) (read={len(docs_list)})"
    )
    return docs_list
...
def create_document(
    ...
    **kwargs,
    ...
    if document_name:
        ...
        logger.info(
            f"[Firestore WRITE] {collection_name}/{document_name} (fields={len(kwargs)}) (write=1)"
        )
    else:
        client_firestore.collection(collection_name).add(kwargs)
        logger.info(
            f"[Firestore WRITE] {collection_name}/__random_token__ (fields={len(kwargs)}) (write=1)"
        )
    return kwargs
...
def delete_document(collection_name: str, document_name: str) -> None:
    ...
    logger.info(f"[Firestore DELETE] {collection_name}/{document_name} (delete=1)")
